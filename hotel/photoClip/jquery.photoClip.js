...
$.fn.photoClip = function(option) {
	...
	$.extend(defaultOption, option);//这里使用的就是$.extend()的嵌套多个对象的功能。
...
function photoClip(container, option) {
	var clipWidth = option.width,
		...
		clipNoPhoto = option.clipNoPhoto,
	...
	$file.change(function() {//file的change事件
		...
		if (!/image\/\w+/.test(this.files[0].type)) {
			...
			//保存文件名
			$view.attr('fileName',this.files[0].name);
	...
	$file.click(function() {
		this.value = "";//清空file的value值，否则上次已加载成功的图片无法再次加载。
	});
	...
	function createImg(src) {
		...
		$img.load(imgLoad);//当指定的元素（及子元素）已加载时，会发生 load() 事件。
	...
	function init() {
		...
		$container = $(container).css({
			"user-select": "none",//none--文本不能被用户选择
			"overflow": "hidden"
		});
		...
		}).appendTo($container);//appendTo() 方法在被选元素的结尾（仍然在内部）插入指定内容。
		...
		var $mask = $("<div class='photo-clip-mask'>").css({
			...
			"width": "100%",
			"height": "100%",
			"pointer-events": "none"//none——元素不再是鼠标事件的目标，鼠标不再监听当前层而去监听下面的层中的元素。但是如果它的子元素设置了pointer-events为其它值，比如auto，鼠标还是会监听这个子元素的。 
		}).appendTo($container);
