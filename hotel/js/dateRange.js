...
*/
	...
	//默认日历参数最大是3
	this.mOpts.calendars = Math.min(this.mOpts.calendars, 1);
	//根据不同主题需要初始化的变量
    ...
    var wrapper = {
		...
		ta:[
			'<div id="' + this.calendarId + '" class="ta_calendar ta_calendar2 cf">',
				...
				'</div>',
				'<div class="ta_calendar_footer cf" '+ (this.mOpts.autoSubmit ? ' style="display:block" ' : '') +'>',
					'<div class="frm_msg">',
					...
					'</div>',
					'<div class="frm_btn">',
						'<button class="ta_btn ta_btn_primary" type="button" name="' + this.submitBtn + '" id="' + this.submitBtn + '" disabled="">确定</button>',
						'<input class="ta_btn" type="button" id="'+ this.closeBtn + '" value="取消" style="display:none"/>',
					'</div>',
				'</div>',
			'</div>'
		]
...
//	if(dateBegin.getMonth()==today.getMonth()){
//		$(".ta_calendar_cont .i_pre").css({"left":100})
//	}
	if(isTaTheme){
...
//			if(dateBegin.getMonth()==today.getMonth()){
//		$(td).append('<a href="javascript:void(0);" id="' + this.preMonth + '"><i class="i_pre h_hide"></i></a>');
//	}
		}
    ...
    for(var d = dateBegin; d.getTime() <= dateEnd.getTime(); d.setDate(d.getDate() + 1)) {
        if(d.getTime() < firstDayOfMonth.getTime()) { // 当前月之前的日期
            ...
            tdClass = this.mOpts.theme + '_' + this.mOpts.disableGray;
            deviation = '1';
        } else if((this.mOpts.stopToday == true && (d.getTime()+86400000) < today.getTime()) || d.getTime() < __method.mOpts.minValidDate * 1000 || ('' !== __method.mOpts.maxValidDate && d.getTime() > __method.mOpts.maxValidDate * 1000)) { // 当前时间之后的日期，或者开启统计之前的日期
            tdClass = this.mOpts.theme + '_' + this.mOpts.disableGray;
            deviation = '2';
        } else { // 当前月日期
            deviation = '0';
